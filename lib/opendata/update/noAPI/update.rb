#平成27年度（平成28年3月31日現在）のデータ。保管前にUTF-8に変換する必要あり
require '../../config.rb'

#CSVファイルの正規化
require "#{$WORKPATH}/update/noAPI/convert.rb"
...
#データベースにインポート
Dir.chdir("#{$WORKPATH}/update/noAPI/convertedFile")
nameArray = Dir.glob("*.csv")
nameArray.each do |fileName|
    tableName = fileName.gsub(".csv", "")
    db.query("load data local infile '#{$WORKPATH}/update/noAPI/convertedFile/#{fileName}' into table #{tableName} FIELDS TERMINATED BY ',' IGNORE 1 LINES;")
end
db.close